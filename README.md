# GITEE 是个坑货，我所有的仓库都吞了，本地有存储的还好处理，本地没有存储的只能哭了。挂在这里对GITEE鞭尸！！！
...
2. 关于`lxml_to_dict`包的问题，此包用于处理企微回调的xml数据，当初选包的时候，比较随心，此包更新比较少，导致很多用户无法正常安装。目前使用`xmltodict`替换`lxml_to_dict` 2020-08-17
