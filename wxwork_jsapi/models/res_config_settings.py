...
class ResConfigSettings(models.TransientModel):
    ...
    jsapi_debug = fields.Boolean(
        "JS API Debug mode", config_parameter="wxwork.jsapi_debug", default=False,
    )
    ...
    ticket_interval_type = fields.Selection(
        related="company_id.ticket_interval_type", readonly=False,
    )

    get_ticket_last_time = fields.Datetime(
        related="company_id.get_ticket_last_time", readonly=False,
    )
    ...
    def get_agent_jsapi_ticket(self):
        ...
        if corpid == False:
            ...
            try:
                ...
                response = wxapi.httpCall(
                    CORP_API_TYPE["GET_TICKET"],
                    {"access_token": api.getAccessToken(), "type": "agent_config",},
                )
