odoo.define('web.Menu', function (require) {
    ...
    var Menu = Widget.extend({
        ...
        events: {
            'click .o_launcher_toggle': '_onToggleLauncherMenu',
        ...
        init: function (parent, menu_data) {
            ...
            this.launcher_menu_displayed = true; //应用抽屉
            this.backbutton_displayed = false;//显示回退按钮
            ...
            core.bus.on('toggle_mode', this, this.toggle_mode); //社区版无
        },
        start: function () {
            var self = this;

            this.$menu_apps = this.$('.o_menu_apps');
            this.$menu_toggle = this.$('.o_launcher_toggle');
            this.$menu_brand_placeholder = this.$('.o_menu_brand');
            this.$section_placeholder = this.$('.o_menu_sections');

            this.$menu_logo = this.$('.o_menu_logo');
            this.$mobile_menu_toggle = this.$('.o_mobile_menu_toggle');
            ...
            dom.initAutoMoreMenu(this.$section_placeholder, {
                maxWidth: function () {
                    // return self.$el.width() - (self.$menu_apps.outerWidth(true) + self.$menu_brand_placeholder.outerWidth(true) + self.systray_menu.$el.outerWidth(true)); //社区版
        ...
        toggle_mode: function (launcher_menu, overapp) {
            ...
            if (this.launcher_menu_displayed && !this.backbutton_displayed) {
            ...
            this.$menu_logo.toggleClass('d-none', this.launcher_menu_displayed);
            this.$mobile_menu_toggle.toggleClass('d-none', this.launcher_menu_displayed);
        },
        change_menu_section: function (primary_menu_id) {
            if (!this.$menu_sections[primary_menu_id]) {
                // this._updateMenuBrand();  //社区版
            ...
            for (var i = 0; i < this.menu_data.children.length; i++) {
                if (this.menu_data.children[i].id === primary_menu_id) {
                    // this._updateMenuBrand(this.menu_data.children[i].name); //社区版
