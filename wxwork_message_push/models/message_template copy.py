...
class MessageTemplate(models.Model):
    ...
    res_id = fields.Char("ResId", readonly=True, help="存储邮件模板的外部扩展id")
    ...
    subject = fields.Char("Subject", help="Subject (placeholders may be used here)")
    ...
    use_default_to = fields.Boolean(
        ...
        "- message (using email_from or message field)",
    )
    ...
    def copy_mail_template(self):
        ...
        for template in templates:
            message = self.search([("name", "=", template.name)], limit=1,)
            if len(message) > 0:
                pass
            else:
                # pass
                self.create(
                    {
                        "name": template.name,
                        "res_id": re.findall(
                            r"'(.*?)'", str(template.get_external_id()), re.DOTALL
                        )[
                            0
                        ],  # get_external_id()获取邮件模板的外部扩展id，即xml中的id
    ...
    def generate_message(self, res_ids, fields=None):
        """
        根据res_ids给定的记录，从给定给定模型的模板生成企业微信通知内容。
        ...
        for lang, (template, template_res_ids) in self._classify_per_lang(
            ...
            print(template, template_res_ids)
            # for field in fields:
            #     for field in fields:
            ...
            #     # 计算企业微信消息的接收人
            #     if any(
            #         field in fields for field in ["email_to", "partner_to", "email_cc"]
            #     ):
