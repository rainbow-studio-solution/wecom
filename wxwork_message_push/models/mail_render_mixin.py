...
class MailRenderMixin(models.AbstractModel):
    ...
    def _render_template_jinja(self, template_txt, model, res_ids, add_context=None):
        ...
        try:
            # print("_render_template_jinja", res_ids, template_txt)
            ...
            return results
        return super(MailRenderMixin, self)._render_template_jinja(
            template_txt, model, res_ids, add_context=None
        )
    ...
    def _render_template(
        ...
        :param post_process: 执行渲染的str / html后处理（请参见``_render_template_postprocess``）

        :return dict: {res_id: 根据记录呈现的模板字符串}
        """
        # print(template_src)

        return super(MailRenderMixin, self)._render_template(
            template_src,
            model,
            res_ids,
            engine="jinja",
    ...
    def _render_field(
        ...
        :param set_lang: 强制语言
        :param post_process: 执行渲染的str / html后处理（请参见_render_template_postprocess）
        ...
        # return super(MailRenderMixin, self)._render_field(
        #     field, res_ids, compute_lang=False, set_lang=False, post_process=False
        # )
