...
class TemplatePreview(models.TransientModel):
    _inherit = "wxwork.message.template"
    _name = "wxwork_message_template.preview"
    ...
    def default_get(self, fields):
        ...
        template = (
            ...
            and self.env["mail.template"].browse(self._context["template_id"])
            or False
        )
        if "res_id" in fields and not result.get("res_id"):
            records = self._get_records()
            result["res_id"] = (
                records and records[0][0] or False
        ...
        if (
            template
            and "preview_lang" in fields
            and not result.get("preview_lang")
            and result.get("res_id")
        ):
            result["preview_lang"] = (
                template.lang
                and template.generate_email(result["res_id"], ["lang"])["lang"]
                or template._context.get("lang")
            )
        return result
    ...
    def on_change_res_id(self):
        ...
        if self._context.get("template_id"):
            ...
            mail_values = template.with_context(
                template_preview_lang=self.preview_lang
            ).generate_email(self.res_id)
        for field in [
            "email_from",
            "email_to",
            "email_cc",
