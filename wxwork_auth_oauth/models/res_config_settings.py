...
class ResConfigSettings(models.TransientModel):
    ...
    auth_agentid = fields.Char('Agent Id', help='The web application ID of the authorizing party, which can be viewed in the specific web application',
                               config_parameter='wxwork.auth_agentid', )
    auth_secret = fields.Char(
        "Secret", config_parameter='wxwork.auth_secret', )
    auth_redirect_uri = fields.Char('Callback link address redirected after authorization', help='Please use urlencode to process the link',
                                    config_parameter='wxwork.auth_redirect_uri')
    qr_redirect_uri = fields.Char('Scan the QR code to log in and call back the link address', help='Please use urlencode to process the link',
