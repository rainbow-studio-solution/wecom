...
class WxWorkMessageComposer(models.TransientModel):
    """
    通用消息编写向导。 您可以在模型和视图级别从此向导继承以提供特定功能。

    向导的行为取决于composition_mode字段：
    -'评论'：发布在记录中。 该向导通过``get_record_data''进行了预填充
    -'mass_mail'：群发邮件模式下的向导，其中邮件详细信息可以包含模板占位符，该模板占位符将在发送给每个收件人之前与实际数据合并。 
    """

    _name = "wxwork.compose.message"
    _description = "Enterprise WeChat message composition wizard"
    _log_access = True
    _batch_size = 500
    ...
    def default_get(self, fields):
        """ 
        处理构图模式。 有关上下文键的一些详细信息：
        - 评论：默认模式，型号和ID记录用户评论
            - default_model或active_model
            - default_res_id或active_id
        - mass_mail: 记录用户群发消息的模型和ID
            - active_ids: record IDs
            - default_model或active_model 
        """
        result = super(WxWorkMessageComposer, self).default_get(fields)

        # 作者
        missing_author = "author_id" in fields and "author_id" not in result
        missing_message_from = "message_from" in fields and "message_from" not in result
        if missing_author or missing_message_from:
            author_id, message_from = self.env["mail.thread"]._message_compute_author(
                result.get("author_id"),
                result.get("message_from"),
                raise_exception=False,
            )
            if missing_message_from:
                result["message_from"] = message_from
            if missing_author:
        ...
        if (
            "no_auto_thread" in fields
            and "no_auto_thread" not in result
        ...
        ):  # 不是context.get（），因为我们要保留 global [] domains
    ...
    author_id = fields.Many2one(
        ...
        "Author",
        index=True,
        help="Author of the message. If not set, message_from may hold an email address that did not match any partner.",
    )
    # composition
    ...
    def get_mail_values(self, res_ids):
        ...
        if mass_mail_mode and not self.no_auto_thread:
            ...
            reply_to_value = records._notify_get_reply_to(default=self.message_from)
        ...
        for res_id in res_ids:
            ...
            if mass_mail_mode and self.model:
                ...
                if self.no_auto_thread and not mail_values.get("reply_to"):
                    mail_values["reply_to"] = mail_values["message_from"]
    ...
    def render_message(self, res_ids):
        ...
        :return dict results: for each res_id, the generated template values for
                              subject, body, message_from and reply_to
        """
        self.ensure_one()
        multi_mode = True
        ...
        emails_from = self.env["mail.render.mixin"]._render_template(
            self.message_from, self.model, res_ids
        )
        ...
        for res_id in res_ids:
            results[res_id] = {
                ...
                "body": bodies[res_id],
                "message_from": emails_from[res_id],
