...
class ResConfigSettings(models.TransientModel):
    ...
    def sync_contacts(self):
        ...
        if not Common(auto_sync).str_to_bool():
            ...
            try:
                department_sync_operate = SyncDepartment(corpid, secret, sync_department_id,
                                                         Department).sync_department()
            ...
            #         raise UserError('提示：企业微信员工绑定系统用户失败')
            #     else:
            #         employee_binding_user_status = True
            # except BaseException:
            #     pass

        if department_sync_status:
            return True
