...
class FinanceSdkTest:
    def __init__(self, corpid, secret, private_keys):
        """
        初始化sdk
        :param corpid: 企业id
        :param secret: 企业会话内容存档密钥
        :param private_keys: 私钥列表，
        """
        # print("__init__", corpid, secret, private_keys)
        self.corpid = corpid
        self.secret = secret
    ...
    def init_sdk(self):
        ...
        for key in self.private_keys:
            ...
            # key_dict["private_key"] = key["private_key"]
            key_dict["private_key"] = PKCS1_v1_5.new(RSA.importKey(key["private_key"]))
            self.ciphers.append(key_dict)
        lib_path = ""
        if platform.system() == "Windows":
            # windows平台
            lib_path = "G:\odoo\\14.0\wxwork_pro\wecom_msgaudit\sdk\windows\WeWorkFinanceSdk.dll"
        else:
            # 非window平台
            lib_path = "/develop/odoo/14.0/wecom_pro/wecom_msgaudit/sdk/linux/libWeWorkFinanceSdk_C.so"
        ...
        result = self.dll.Init(c_void_p(self.sdk), c_char_p(corpid), c_char_p(secret))
        if result != 0:
