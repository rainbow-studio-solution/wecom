...
class SendWxWorkMessage(models.TransientModel):
    _name = "wxwork.message.composer"
    _description = "Send Enterprise WeChat message Wizard"

    @api.model
    def default_get(self, fields):
        result = super(SendWxWorkMessage, self).default_get(fields)
    ...
    composition_mode = fields.Selection(
        [
            ("touser", "Send to Enterprise WeChat staff"),
            ("toparty", "Send to Enterprise WeChat department"),
            ("totag", "Send to Enterprise WeChat tag"),
            ("comment", "Post on a document"),
            ("mass", "Send Enterprise WeChat message in batch"),
        ],
    ...
    # options for comment and mass mode 评论和大众模式的选项
    ...
    recipient_single_number_itf = fields.Char(
        ...
        compute_sudo=False,
        store=True,
        help="UX字段，允许编辑收件人号码。 如果更改，它将存储到收件人上。",
    )
    ...
    def create(self, values):
        ...
        return super(SendWxWorkMessage, self).create(values)
    ...
    def _action_send_sms_numbers(self):
        self.env["sms.api"]._send_sms_batch(
            [
                {"res_id": 0, "number": number, "content": self.body,}
