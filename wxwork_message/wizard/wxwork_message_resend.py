...
class WxWorkMessageRecipient(models.TransientModel):
    # 重新发送通知
    _name = "wxwork.message.resend.recipient"
    _description = "Resend Notification"
    _rec_name = "wxwork_message_resend_id"

    wxwork_message_resend_id = fields.Many2one("wxwork.message.resend", required=True)
...
class WxWorkMessageResend(models.TransientModel):
    _name = "wxwork.message.resend"
    _description = "Enterprise WeChat message Resend"
    _rec_name = "mail_message_id"

    @api.model
    def default_get(self, fields):
        result = super(WxWorkMessageResend, self).default_get(fields)
    ...
    recipient_ids = fields.One2many(
        "wxwork.message.resend.recipient",
        "wxwork_message_resend_id",
        string="Recipients",
    )
    ...
    def action_resend(self):
        ...
        if to_resend_ids:
            ...
            for pid, cid, active, pshare, ctype, notif, groups in self.env[
                "mail.followers"
            ]._get_recipient_data(record, "wxwork", False, pids=pids):
                if pid and notif == "wxwork":
                    rdata.append(
                        {
                            "id": pid,
                            "share": pshare,
