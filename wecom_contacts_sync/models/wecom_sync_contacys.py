...
class WecomSyncContactsTask(models.TransientModel):
    _name = "wecom.sync.contacts.task"
    ...
    employee_sync_times = fields.Float(
        string="Employee synchronization time (seconds)",
        digits=(16, 3),
        readonly=True,
    )

    tag_sync_result = fields.Boolean(
        string="Tag synchronization results",
        default=False,
        readonly=True,
    )
    tag_sync_times = fields.Float(
        string="Tag synchronization time (seconds)",
        digits=(16, 3),
        readonly=True,
    )

    sync_result = fields.Boolean(
        string="Synchronization results",
        default=False,
        readonly=True,
    )
    sync_times = fields.Float(
        string="Synchronization time (seconds)",
        digits=(16, 3),
        readonly=True,
    )
    sync_failure_reason = fields.Text(
        'Sync Failure Reason', readonly=1)
    ...
    def run(self, company):
        ...
        if company.contacts_app_id:
            ...
            if sync_hr_enabled == "False" or sync_hr_enabled is None:
                task.write({
                    "sync_result":  _("company [%s] does not allow synchronization.") % (company.name)
                    })
                _logger.warning(
                        _(
